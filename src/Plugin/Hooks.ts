...
export function registerHook(hookType: HookType): IHook {
  ...
  switch (hookType) {
    case HookType.RegisterForBatteryStateChangesPseudo:
      // Pass context
      hook.unregister = CreatePseudoBatteryStateRegistry(context)
      break;
...
import { BatteryStateEvent } from "./../Alarms/Battery/Battery.interfaces"
import { events } from "./BackendContext.h"
function CreatePseudoBatteryStateRegistry(context: any) {
  ...
  context.intervalID = setInterval(() => {
    ...
    context.eventBus.dispatchEvent(new BatteryStateEvent(events.map.BatteryState, { detail: context.batteryState }))
  }, 1000)
